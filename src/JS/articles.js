const firebaseConfig = {
    ...
    // User's saved articles (persistent)
    let userSavedArticles = JSON.parse(localStorage.getItem('vt_user_saved_articles') || '[]');

    function saveUserArticlesToLocal() {
        localStorage.setItem('vt_user_saved_articles', JSON.stringify(userSavedArticles));
    }

    function showToast(msg, isError = false) {
        const toast = document.createElement('div');
        toast.className = `toast ${isError ? 'bg-rose-700' : 'bg-slate-800'}`;
    ...
    async function deleteArticle(articleId) {
        try {
            ...
            // Also remove from user saved if exists
    ...
    async function updateArticle(articleId, updates) {
        ...
        const index = articlesArray.findIndex(a => a.id === articleId);
        if (index !== -1) articlesArray[index] = { ...articlesArray[index], ...updates };
        renderLibrary();
        renderAdminList();
    ...
    function getPdfUrl(article) {
        if (article.isExternal) return article.pdfUrl;
        if (article.pdfData) return article.pdfData;
        return null;
    }
    ...
    function renderLibrary() {
        ...
        if (!grid) return;
        if (filtered.length === 0) grid.innerHTML = '<div class="col-span-3 text-center py-16 text-slate-400">No matching articles.</div>';
        else {
            grid.innerHTML = filtered.map(a => `
                <div class="glass-card p-5">
                    <div class="flex justify-between items-start"><h3 class="font-bold text-xl text-slate-800">${escapeHtml(a.title)}</h3><i class="fas fa-file-pdf text-indigo-400 text-2xl"></i></div>
                    <p class="text-slate-500 text-sm mt-2">${escapeHtml((a.content || '').substring(0,100))}...</p>
                    <div class="flex gap-2 mt-3"><span class="text-xs bg-indigo-50 text-indigo-700 px-2 py-1 rounded-full">${escapeHtml(a.area || 'General')}</span></div>
                    <div class="mt-4 flex justify-between items-center">
                        <span class="text-xs text-slate-400">${new Date(a.createdAt).toLocaleDateString()}</span>
                        <div class="flex gap-2">
                            <button class="read-btn bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-3 py-1.5 rounded-full" data-url="${escapeHtml(getPdfUrl(a))}" data-title="${escapeHtml(a.title)}"><i class="fas fa-eye mr-1"></i> Read</button>
                            <button class="save-to-dashboard-btn bg-amber-500 hover:bg-amber-600 text-white text-sm px-3 py-1.5 rounded-full" data-id="${a.id}"><i class="fas fa-bookmark mr-1"></i> Save</button>
                        </div>
                    </div>
                </div>`).join('');
        }
    ...
    // Render User Dashboard (saved articles)
    function renderDashboard() {
        ...
        savedGrid.innerHTML = userSavedArticles.map(save => {
            // find full article data to display details (might be deleted from main library but still show saved)
            const fullArticle = articlesArray.find(a => a.id === save.id);
            const title = fullArticle ? fullArticle.title : save.title;
            const area = fullArticle ? (fullArticle.area || 'General') : (save.area || 'Legal');
            const pdfData = save.pdfData || (fullArticle ? getPdfUrl(fullArticle) : null);
            return `
                <div class="glass-card p-5 saved-card">
                    <div class="flex justify-between items-start"><h3 class="font-bold text-xl text-slate-800">${escapeHtml(title)}</h3><i class="fas fa-check-circle text-green-500 text-xl"></i></div>
                    <p class="text-slate-500 text-sm mt-2">Saved to your personal library</p>
                    <div class="flex gap-2 mt-3"><span class="text-xs bg-green-50 text-green-700 px-2 py-1 rounded-full">${escapeHtml(area)}</span></div>
                    <div class="mt-4 flex justify-between items-center">
                        <span class="text-xs text-slate-400">${new Date(save.savedAt).toLocaleDateString()}</span>
                        <div class="flex gap-2">
                            <button class="read-saved-btn bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-3 py-1.5 rounded-full" data-url="${escapeHtml(pdfData)}" data-title="${escapeHtml(title)}"><i class="fas fa-eye mr-1"></i> Read</button>
                            <button class="remove-saved-btn bg-red-500 hover:bg-red-600 text-white text-sm px-3 py-1.5 rounded-full" data-id="${save.id}"><i class="fas fa-trash-alt mr-1"></i> Remove</button>
                        </div>
                    </div>
                </div>`;
        }).join('');
        ...
        document.querySelectorAll('.remove-saved-btn').forEach(btn => btn.addEventListener('click', () => {
            ...
            userSavedArticles = userSavedArticles.filter(s => s.id !== id);
            saveUserArticlesToLocal();
            renderDashboard();
    ...
    // Save article to user dashboard (triggered after download key success)
    function saveToUserDashboard(articleId) {
        ...
        const savedItem = {
            ...
            title: article.title,
            area: article.area,
            pdfData: getPdfUrl(article),
            savedAt: new Date().toISOString()
        };
        userSavedArticles.push(savedItem);
        saveUserArticlesToLocal();
        renderDashboard();
        showToast(`📘 "${article.title}" saved to your dashboard!`);
        return true;
    }

    async function openPdfViewer(url, title) {
        document.getElementById('pdfTitle').innerHTML = escapeHtml(title);
        ...
        try {
            ...
            currentScale = 1.3;
            await renderPage();
        } catch(e) { showToast("Error loading PDF", true); }
    }
    ...
    function renderAdminList() {
        ...
        if (!container) return;
        if (articlesArray.length === 0) { container.innerHTML = '<div class="text-center text-slate-400 py-5">No articles</div>'; return; }
        container.innerHTML = articlesArray.map(a => `
            <div class="bg-white border rounded-xl p-3 flex justify-between items-center">
                <div><span class="font-semibold">${escapeHtml(a.title)}</span><span class="text-xs ml-2 bg-slate-100 px-2 py-0.5 rounded-full">${escapeHtml(a.area)}</span></div>
                <div><button class="edit-admin-btn bg-indigo-50 text-indigo-700 px-3 py-1 rounded-lg text-sm" data-id="${a.id}">Edit</button>
                <button class="delete-admin-btn bg-rose-50 text-rose-600 px-3 py-1 rounded-lg text-sm ml-1" data-id="${a.id}">Delete</button></div>
            </div>`).join('');
        document.querySelectorAll('.edit-admin-btn').forEach(btn => btn.addEventListener('click', async () => {
            const id = btn.dataset.id;
            const art = articlesArray.find(a => a.id === id);
            if (art) {
                const newTitle = prompt('Title:', art.title);
                if (newTitle) await updateArticle(id, { title: newTitle, content: prompt('Desc:', art.content) || art.content, area: prompt('Area:', art.area) || art.area });
            }
        }));
    ...
    if (dropZone) {
        ...
        dropZone.addEventListener('drop', e => {
            ...
            if (file && file.type === 'application/pdf') {
                ...
                reader.onload = ev => { selectedFileBase64 = ev.target.result; uploadStatus.innerHTML = `<span class="text-green-600">✅ ${file.name} ready</span>`; };
                reader.readAsDataURL(file);
            } else { uploadStatus.innerHTML = '<span class="text-rose-600">❌ PDF required</span>'; }
        });
        fileInput.addEventListener('change', e => { if(e.target.files[0]){ const reader = new FileReader(); reader.onload = ev => { selectedFileBase64 = ev.target.result; uploadStatus.innerHTML = `<span class="text-green-600">✅ ${e.target.files[0].name} loaded</span>`; }; reader.readAsDataURL(e.target.files[0]); } });
    }
    document.getElementById('uploadArticleBtn')?.addEventListener('click', async () => {
        const title = document.getElementById('newTitle').value.trim();
        const content = document.getElementById('newContent').value.trim();
        if (!title || !content) return showToast('Title and description required', true);
        if (!selectedFileBase64) return showToast('Select a PDF', true);
        if (selectedFileBase64.length > 1.2e6) return showToast('PDF exceeds 1MB', true);
        await createArticle(title, content, document.getElementById('newArea').value, document.getElementById('newKeywords').value, selectedFileBase64);
        document.getElementById('newTitle').value = ''; document.getElementById('newContent').value = ''; document.getElementById('newArea').value = ''; document.getElementById('newKeywords').value = ''; selectedFileBase64 = null; fileInput.value = ''; uploadStatus.innerHTML = '';
    });
    ...
    // Download/Save to Dashboard logic
    document.getElementById('confirmDownloadBtn')?.addEventListener('click', async () => {
        const key = document.getElementById('downloadKeyInput').value;
        const res = await verifyDownloadPassword(key);
        if (res.success) {
            if (pendingDownloadId) {
                saveToUserDashboard(pendingDownloadId);
                document.getElementById('downloadModal').classList.add('hidden');
                document.getElementById('downloadKeyInput').value = '';
                document.getElementById('downloadError').classList.add('hidden');
            }
        } else {
            document.getElementById('downloadError').innerText = 'Invalid download key';
    ...
    document.getElementById('navAdminBtn')?.addEventListener('click', () => adminModal.classList.remove('hidden'));
    ...
    document.getElementById('loginAdminBtn')?.addEventListener('click', async () => {
        ...
        if (res.success) { document.getElementById('adminLoginArea').classList.add('hidden'); document.getElementById('adminContentArea').classList.remove('hidden'); renderAdminList(); showToast('Admin access granted'); }
        else { document.getElementById('loginError').innerText = 'Invalid password'; document.getElementById('loginError').classList.remove('hidden'); }
    });
    document.getElementById('resetPasswordsBtn')?.addEventListener('click', resetPasswordsToDefault);
    document.getElementById('updateAdminPassBtn')?.addEventListener('click', async () => {
        const old = document.getElementById('oldAdminPass').value, np = document.getElementById('newAdminPass').value, cf = document.getElementById('confirmAdminPass').value;
        if(np !== cf) { document.getElementById('adminPassMsg').innerHTML = '<span class="text-rose-600">Mismatch</span>'; return; }
        const res = await updateAdminPassword(old, np);
        document.getElementById('adminPassMsg').innerHTML = `<span class="${res.success ? 'text-green-600' : 'text-rose-600'}">${res.message}</span>`;
    });
    document.getElementById('updateDownloadPassBtn')?.addEventListener('click', async () => {
        const old = document.getElementById('oldDownloadPass').value, np = document.getElementById('newDownloadPass').value, cf = document.getElementById('confirmDownloadPass').value;
        if(np !== cf) { document.getElementById('downloadPassMsg').innerHTML = '<span class="text-rose-600">Mismatch</span>'; return; }
        const res = await updateDownloadPassword(old, np);
        document.getElementById('downloadPassMsg').innerHTML = `<span class="${res.success ? 'text-green-600' : 'text-rose-600'}">${res.message}</span>`;
    });
